...
public class PlayerColonelloCorrections{
   ...
   private char[] rerollsChoice = new char[5];
   ...
   //constructors
  	 public PlayerColonelloCorrections()
  	 {
  	 }
   	 public PlayerColonelloCorrections(String name)
   	 {
       this.name=name;
   	}
   ...
   //1. This method will reroll the dice based on user input.
   public void reroll(){
        ...
        do{
        	...
        	reroll = validateReroll(sc.nextLine());

        	if(reroll.equalsIgnoreCase("reroll"))
        	{
   			System.out.println("\n\t\tType:\"y\" to reroll and \"n\" to keep the current value:");
        		for(int i = 0; i < diceValues.length; i++)
        		{
   				System.out.print("\tDice "+(i+1)+": ");
   				rerollsChoice[i] = sc.nextLine().charAt(0);

   				//validate user input for yes or no
   				while(!(rerollsChoice[i] == 'y'||rerollsChoice[i] == 'n')){
   					System.out.println("\n\tERROR! Please enter \"y\" or \"n\": ");
   					rerollsChoice[i]=sc.nextLine().charAt(0);
   				}

   				//if statement to reroll certain dice
   					if(rerollsChoice[i] == 'y')
   						diceValues[i] = (int)(Math.random()*6+1);
   			}
   		...
   		count++;
	   }while(!(reroll.equalsIgnoreCase("done")|| count>=2));
   }

   //2. This method will display the scorecard with its current values.
